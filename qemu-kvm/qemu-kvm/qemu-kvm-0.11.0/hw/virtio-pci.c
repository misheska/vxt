...
fprintf(stderr, "CDY VIRTIO_MSI_CONFIG_VECTOR, call msix_vector_use, dev %p, val 0x%x\n", &proxy->vdev, val);
...
fprintf(stderr, "CDY virtio_pci_irqfd -EINVAL\n");
        return -EINVAL;
    }
    if (!proxy->pci_dev.msix_entry_used[vector]) {
fprintf(stderr, "CDY virtio_pci_irqfd -ENOENT, dev %p, vector 0x%x\n", &proxy->pci_dev, vector);
        return -ENOENT;
    }
    call.fd = fd;
    call.gsi = proxy->pci_dev.msix_irq_entries[vector].gsi;
fprintf(stderr, "CDY virtio_pci_irqfd vector 0x%x, gsi 0x%x\n",
        vector, call.gsi);
...
fprintf(stderr, "CDY virtio_init_pci vdev = %p, nvectors = 0x%x\n",
        vdev, vdev->nvectors);
...
fprintf(stderr, "CDY virtio_init_pci after pci_config_set_class\n");
...
fprintf(stderr, "CDY virtio_init_pci pci_register_bar called on SPACE_MEM\n");
        pci_register_bar(&proxy->pci_dev, 1,
                         msix_bar_size(&proxy->pci_dev),
                         PCI_ADDRESS_SPACE_MEM,
                         msix_mmio_map);
fprintf(stderr, "CDY virtio_init_pci after pci_register_bar called on SPACE_MEM\n");
...
fprintf(stderr, "CDY virtio_init_pci pci_register_bar called on SPACE_IO\n");
    pci_register_bar(&proxy->pci_dev, 0, size, PCI_ADDRESS_SPACE_IO,
                           virtio_map);
fprintf(stderr, "CDY virtio_init_pci after pci_register_bar called on SPACE_IO\n");
...
static void virtio_vxt_init_pci(PCIDevice *pci_dev)
{
    VirtIOPCIProxy *proxy = DO_UPCAST(VirtIOPCIProxy, pci_dev, pci_dev);
    VirtIODevice *vdev;

fprintf(stderr, "CDY virtio_vxt_init_pci\n");
    vdev = virtio_vxt_init(&pci_dev->qdev);

    vdev->nvectors = proxy->nvectors;

    virtio_init_pci(proxy, vdev,
/*  We can't have our own unless we have a registered id and want to write a 
    virtio_pci equivalent.
                    PCI_VENDOR_ID_SYMANTEC_VXT,
*/
                    PCI_VENDOR_ID_REDHAT_QUMRANET,
                    PCI_DEVICE_ID_VIRTIO_VXT_BUS,
...
static PCIDeviceInfo virtio_info[] = {
    {
        ...
        .qdev.name = "virtio-vxt-pci",
        .qdev.size = sizeof(VirtIOPCIProxy),
        .init      = virtio_vxt_init_pci,
        .qdev.props = (Property[]) {
            {
                .name   = "vectors",
                .info   = &qdev_prop_uint32,
...
{
fprintf(stderr, "CDY virtio_pci_register_devices\n");
